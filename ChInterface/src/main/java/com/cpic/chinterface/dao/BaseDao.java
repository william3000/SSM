...
////		int totalCount = ((Integer) this.getSqlSessionTemplate().selectOne(this.getNamespace().concat(sqlCountId),
////				params)).intValue();
...
////				RowBounds rowBounds = new RowBounds(pager.getPageSize() * (pager.getCurPage() - 1),
////						pager.getPageSize());
////				List resultRows = this.getSqlSessionTemplate().selectList(this.getNamespace().concat(sqlId), params,
////						rowBounds);
////				this.logger.debug("本次查询第" + pager.getCurPage() + "页，每页大小设定" + pager.getPageSize() + "条,本次返回条数为："
////						+ resultRows.size());
...
//		List resultRows = this.getSqlSessionTemplate().selectList(this.getNamespace().concat(sqlId), params, rowBounds);
...
//		this.logger.debug(sqlId + "<<<<<start>>>>>");
//		long startmils = System.currentTimeMillis();
//		List list = this.findListByParams(sqlId, params);
...
//		this.logger.debug(sqlId + "<<<<<start>>>>>");
//		long startmils = System.currentTimeMillis();
//		Integer obj = Integer.valueOf(this.sqlSessionTemplate.insert(this.getNamespace().concat(sqlId), params));
//		long startmils3 = System.currentTimeMillis();
//		long offmils = startmils3 - startmils;
//		if (offmils >= (long) this.warningTime) {
...
//		List resultRows = this.findList("findBySelective", 2, 1, params);
//		if (resultRows.size() > 1) {
//			throw new RuntimeException("数据并不唯一，原参数：" + params);
//		} else {
//			return resultRows.size() == 1 ? (T) resultRows.get(0) : null;
//		}
//	}
}
